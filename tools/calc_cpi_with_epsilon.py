...
def main():
    ...
    if args.mode in ("throughput", "both"):
        ...
        print(f"{'OPCODE':<16} {'COUNT':>8} {'AVG_CPI':>10} {'MED_CPI':>10} {'MIN':>8} {'MAX':>8} {'VAR_CPI':>12}")
        for op in sorted(cpi_by_op.keys()):
            xs = cpi_by_op[op]
            var_cpi = safe_variance(xs)
            print(f"{op:<16} {len(xs):>8} {mean(xs):>10.2f} {median(xs):>10.2f} "
                  f"{min(xs):>8.2f} {max(xs):>8.2f} {var_cpi:>12.4f}")
        print()
    ...
    if args.mode in ("latency", "both"):
        ...
        print(f"{'OPCODE':<16} {'COUNT':>8} {'AVG_LAT':>10} {'MED_LAT':>10} {'MIN':>8} {'MAX':>8} {'VAR_LAT':>12}")
        for op in sorted(lat_by_op.keys()):
            xs = lat_by_op[op]
            var_lat = safe_variance(xs)
            print(f"{op:<16} {len(xs):>8} {mean(xs):>10.2f} {median(xs):>10.2f} "
                  f"{min(xs):>8.2f} {max(xs):>8.2f} {var_lat:>12.4f}")
