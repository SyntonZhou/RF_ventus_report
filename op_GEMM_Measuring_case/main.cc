...
int main(void){
    ...
    snprintf(buildopts,sizeof(buildopts),
             ...
             "-DTILE_M=%d -DTILE_N=%d -DTILE_K=%d "
             "-DPERSIST_GX=%d -DPERSIST_GY=%d -DASSUME_ALIGNED=%d",
             TILE_M, TILE_N, TILE_K,
             PERSIST_GX, PERSIST_GY, 1);
    ...
    cl_uint nargs = 0;
    clGetKernelInfo(k, CL_KERNEL_NUM_ARGS, sizeof(nargs), &nargs, NULL);
    printf("[DBG] kernel num args = %u\n", nargs);
    ...
    size_t global[2] = { (size_t)PERSIST_GX * TILE_N, (size_t)PERSIST_GY * TILE_M };
